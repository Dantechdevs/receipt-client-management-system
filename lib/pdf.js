...
// Hardcoded per your answer — single-company install, no DB columns for these.
const COMPANY_TAGLINE = 'POWER • ENERGY • ENGINEERING';
const COMPANY_SERVICES_LINE = 'Electrical • Solar • Plumbing • Construction Solutions';
...
// Splits an item description into a bold title line + a normal body line,
// the way the reference quotation lays out each numbered scope item.
// Convention: first line before "\n" is the title; everything after is body.
function splitItemText(description) {
  const idx = (description || '').indexOf('\n');
  if (idx === -1) return { title: description || '', body: '' };
  return { title: description.slice(0, idx), body: description.slice(idx + 1).trim() };
}
...
async function buildDocumentPDF({ res, company, doc, items, baseUrl, duplicate = false, docType, config, clauses = [], parties = [], stream = true }) {
  ...
  // Quotations drop the top-right contact box, so the name/tagline column can run wider.
  const nameW = isQuotation ? contentW - logoSize - 14 : contentW * 0.5;
  pdf.font('Helvetica-Bold').fontSize(17).fillColor(NAVY).text((company.name || 'Company Name').toUpperCase(), nameX, y, { width: nameW });

  if (isQuotation) {
    let taglineY = pdf.y + 2;
    pdf.font('Helvetica-Bold').fontSize(9).fillColor(ORANGE).text(COMPANY_TAGLINE, nameX, taglineY, { width: nameW });
    pdf.font('Helvetica').fontSize(8).fillColor(TEXT_MUTED).text(COMPANY_SERVICES_LINE, nameX, pdf.y + 1, { width: nameW });
  } else {
    const contactW = 220;
  ...
  const infoH = 22;
  if (isQuotation) {
    // Quotations show two fields only: doc number and date — no phone column.
    const halfW = contentW / 2;
    pdf.rect(margin, y, contentW, infoH).lineWidth(1).strokeColor(BORDER).stroke();
    pdf.moveTo(margin + halfW, y).lineTo(margin + halfW, y + infoH).stroke();
    pdf.font('Helvetica-Bold').fontSize(8.5).fillColor(TEXT_DARK);
    pdf.text(`${config.label.toUpperCase()} NO: ${doc.doc_number}`, margin + 8, y + 7, { width: halfW - 16 });
    pdf.text(`DATE: ${fmtDate(doc.issue_date)}`, margin + halfW + 8, y + 7, { width: halfW - 16 });
  } else {
  ...
  pdf.font('Helvetica-Bold').fontSize(8.5).fillColor(TEXT_DARK)
    .text(isQuotation ? 'TO (CLIENT): ' : 'NAME: ', margin + 8, y + 6, { continued: true })
    .font('Helvetica').text(doc.customer_name);
  y += fieldH;
  pdf.rect(margin, y, contentW, fieldH).lineWidth(1).strokeColor(BORDER).stroke();
  pdf.font('Helvetica-Bold').fontSize(8.5).fillColor(TEXT_DARK)
    .text(isQuotation ? 'LOCATION: ' : 'ADDRESS: ', margin + 8, y + 6, { continued: true })
    .font('Helvetica').text(doc.customer_address || '—');
  y += fieldH;
  ...
  // Quotation-only PROJECT DETAILS block, backed by documents.project_title / documents.project_ref.
  if (isQuotation && (doc.project_title || doc.project_ref)) {
    const lines = [];
    if (doc.project_title) lines.push(`PROJECT: ${doc.project_title}`);
    if (doc.project_ref) lines.push(`RE: ${doc.project_ref}`);
    const blockText = lines.join('\n');
    const blockH = pdf.heightOfString(blockText, { width: contentW - 16, font: 'Helvetica-Bold', fontSize: 9 }) + 20;

    pdf.font('Helvetica-Bold').fontSize(9.5).fillColor(NAVY).text('PROJECT DETAILS:', margin, y);
    y = pdf.y + 3;
    pdf.rect(margin, y, contentW, blockH).lineWidth(1).strokeColor(BORDER).stroke();
    pdf.font('Helvetica-Bold').fontSize(9).fillColor(TEXT_DARK).text(blockText, margin + 8, y + 6, { width: contentW - 16 });
    y += blockH + 10;
  }

  if (items && items.length) {
    let colNumW, colQtyW, colPriceW, colAmtW, colDescW;
    let colNumX, colQtyX, colDescX, colPriceX, colAmtX;

    if (isQuotation) {
      // Reference layout: #, DESCRIPTION, QTY, RATE, AMOUNT
      colNumW = 26; colQtyW = 45; colPriceW = 95; colAmtW = 100;
      colNumX = margin;
      colDescX = colNumX + colNumW;
      colDescW = contentW - colNumW - colQtyW - colPriceW - colAmtW;
      colQtyX = colDescX + colDescW;
      colPriceX = colQtyX + colQtyW;
      colAmtX = colPriceX + colPriceW;
    } else {
      colQtyW = 55; colPriceW = 95; colAmtW = 100;
      colQtyX = margin;
      colDescX = colQtyX + colQtyW;
      colDescW = contentW - colQtyW - colPriceW - colAmtW;
      colPriceX = colDescX + colDescW;
      colAmtX = colPriceX + colPriceW;
    }
    ...
    function drawTableHeader(yy) {
      ...
      if (isQuotation) {
        pdf.text('#', colNumX, yy + 7, { width: colNumW, align: 'center' });
        pdf.text('DESCRIPTION', colDescX + 6, yy + 7, { width: colDescW - 10 });
        pdf.text('QTY', colQtyX, yy + 7, { width: colQtyW, align: 'center' });
        pdf.text('RATE (KSh)', colPriceX, yy + 7, { width: colPriceW - 8, align: 'right' });
        pdf.text('AMOUNT (KSh)', colAmtX, yy + 7, { width: colAmtW - 8, align: 'right' });
        pdf.lineWidth(1).strokeColor(BORDER);
        pdf.moveTo(colDescX, yy).lineTo(colDescX, yy + 22).stroke();
        pdf.moveTo(colQtyX, yy).lineTo(colQtyX, yy + 22).stroke();
    ...
    items.forEach((item, idx) => {
      let rowHeight;
      let title = item.description, body = '';
      if (isQuotation) {
        const split = splitItemText(item.description);
        title = split.title; body = split.body;
        const titleH = pdf.heightOfString(title, { width: colDescW - 10, font: 'Helvetica-Bold', fontSize: 9 });
        const bodyH = body ? pdf.heightOfString(body, { width: colDescW - 10, font: 'Helvetica', fontSize: 8.5 }) + 3 : 0;
        rowHeight = Math.max(20, titleH + bodyH + 10);
      } else {
        const descH = pdf.heightOfString(item.description, { width: colDescW - 10, font: 'Helvetica', fontSize: 9 });
        rowHeight = Math.max(20, descH + 10);
      }
      ...
      if (isQuotation) {
        ...
        pdf.moveTo(colQtyX, y).lineTo(colQtyX, y + rowHeight).stroke();
        pdf.moveTo(colPriceX, y).lineTo(colPriceX, y + rowHeight).stroke();
        pdf.moveTo(colAmtX, y).lineTo(colAmtX, y + rowHeight).stroke();

        pdf.font('Helvetica').fontSize(9).fillColor(TEXT_DARK);
        pdf.text(String(idx + 1), colNumX, y + 5, { width: colNumW, align: 'center' });
        pdf.font('Helvetica-Bold').fontSize(9).text(title, colDescX + 6, y + 5, { width: colDescW - 10 });
        if (body) pdf.font('Helvetica').fontSize(8.5).fillColor(TEXT_MUTED).text(body, colDescX + 6, pdf.y + 1, { width: colDescW - 10 });
        pdf.font('Helvetica').fontSize(9).fillColor(TEXT_DARK).text(String(item.quantity), colQtyX, y + 5, { width: colQtyW, align: 'center' });
  ...
  // Quotation-only BANK DETAILS box, backed by company.bank_account_name / bank_account_number.
  if (isQuotation && (company.bank_account_name || company.bank_account_number)) {
    const lines = ['BANK DETAILS'];
    if (company.bank_account_name) lines.push(`Account Name: ${company.bank_account_name}`);
    if (company.bank_account_number) lines.push(`Account Number: ${company.bank_account_number}`);
    lines.push(`Amount Due: ${company.currency || 'KES'} ${fmtMoney(doc.total)}`);
    const bodyText = lines.slice(1).join('\n');
    const bodyH = pdf.heightOfString(bodyText, { width: contentW - 16, font: 'Helvetica', fontSize: 9 });
    const blockH = 22 + bodyH + 12;

    if (y + blockH > pageH - FOOTER_H - 230) { newPage(); y = 40; }

    pdf.rect(margin, y, contentW, blockH).lineWidth(1).strokeColor(BORDER).stroke();
    pdf.rect(margin, y, contentW, 20).fill(NAVY);
    pdf.font('Helvetica-Bold').fontSize(9).fillColor('#ffffff').text(lines[0], margin + 8, y + 6);
    pdf.font('Helvetica').fontSize(9).fillColor(TEXT_DARK).text(bodyText, margin + 8, y + 26, { width: contentW - 16 });
    y += blockH + 10;
  }

  // Quotation-only PAYMENT TERMS box. Reuses the existing `clauses[]` mechanism —
  // pass a clause with title 'Payment Terms' and each milestone on its own line in `body`.
  if (isQuotation && clauses && clauses.length) {
    clauses.forEach((clause) => {
      const bodyText = clause.body || '';
      const bodyH = pdf.heightOfString(bodyText, { width: contentW - 16, font: 'Helvetica', fontSize: 9 });
      const blockH = 22 + bodyH + 12;
      if (y + blockH > pageH - FOOTER_H - 230) { newPage(); y = 40; }
      ...
      pdf.rect(margin, y, contentW, blockH).lineWidth(1).strokeColor(BORDER).stroke();
      pdf.rect(margin, y, contentW, 20).fill(NAVY);
      pdf.font('Helvetica-Bold').fontSize(9).fillColor('#ffffff').text((clause.title || 'TERMS').toUpperCase(), margin + 8, y + 6);
      pdf.font('Helvetica').fontSize(9).fillColor(TEXT_DARK).text(bodyText, margin + 8, y + 26, { width: contentW - 16 });
      y += blockH + 10;
    });
  } else if (!isQuotation && clauses && clauses.length) {
  ...
  // Quotations get named Client/Contractor signature columns (same mechanism agreements
  // already use via `parties`), instead of the generic two-line signature block.
  if ((docType === 'agreement' || isQuotation) && parties && parties.length) {
